...
export class NgbdModalAddProjectFileComponent implements OnInit {
  ...
  private refreshProjectFileEntries(): void {
    this.userFileService.retrieveDashboardUserFileEntryList()
      .pipe(untilDestroyed(this))
      .subscribe(dashboardFileEntries => {
        this.unaddedFiles = dashboardFileEntries.filter(fileEntry => fileEntry.file.fid !== undefined && !this.addedFileKeys.has(fileEntry.file.fid!));
